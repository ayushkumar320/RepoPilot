...
This file is the **primary, always-loaded set of project rules** for RepoPilot. Every Codex prompt and every contributor should treat it as authoritative. Keep it concise: deep design detail lives in [`docs/`](docs/) — this file is the index and the rules, not the encyclopedia.
...
| **Harness config** | [`.claude/`](.claude/) | ✅ Yes | Harness-specific wiring: settings and permissions. |
...
**Interaction model:** `AGENTS.md` defines the rules → `.agents/rules/` mirrors them for other tools → `.claude/` carries harness wiring. Per-user memory is personal and additive; it must never contradict this file.
