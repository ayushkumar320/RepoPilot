...
|---|---|---|---|
| **Project rules** | `AGENTS.md` (this file) | ✅ Yes | Single source of truth. Auto-loaded by Codex every session. |
...
| **Harness config** | [`.Codex/`](.Codex/) | ✅ Yes | Codex-specific wiring: hooks, subagents, slash commands. See §5. |
...
| **Per-user memory** | `~/.Codex/…/memory/` | ❌ No (local) | Your private notes across sessions. Never holds shared project rules — those belong here. |

**Interaction model:** `AGENTS.md` defines the rules → `.agents/rules/` mirrors them for other tools → `.Codex/` enforces them via hooks/subagents/commands → Graphify indexes *all* of the above so the rules are themselves queryable. Per-user memory is personal and additive; it must never contradict this file.
...
Only read raw source files **after** Graphify has oriented you, or to modify/debug specific lines. A `PreToolUse` hook in [`.Codex/settings.json`](.Codex/settings.json) enforces this for `Read`/`Glob`/`grep`. **This rule applies to subagents too** — include it in every subagent prompt that explores code (the [`codebase-explorer`](.Codex/agents/codebase-explorer.md) subagent already does).
...
This keeps the shared graph honest. Use the [`/graph-update`](.Codex/commands/graph-update.md) slash command to run it and emit the status block in one step.
...
## 5. Codex harness (`.Codex/`)

| Path | Purpose |
|---|---|
| [`.Codex/settings.json`](.Codex/settings.json) | Hooks enforcing "graph before grep/read"; permission allow-list for `graphify` commands so they don't prompt. |
| [`.Codex/agents/codebase-explorer.md`](.Codex/agents/codebase-explorer.md) | Read-only explorer subagent. Always consults Graphify first; use it to answer architecture questions without polluting the main context. |
| [`.Codex/commands/graph-update.md`](.Codex/commands/graph-update.md) | `/graph-update` — runs `graphify update .`, verifies, stages graph files, emits GRAPH STATUS. |
...
2. **Edit the right layer** (§2): shared rules → here + `.agents/rules/`; harness wiring → `.Codex/`; design depth → `docs/`.
3. **Keep `AGENTS.md` and `.agents/rules/project.md` in sync** — they say the same things for different tools.
