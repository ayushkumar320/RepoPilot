...
## Corpus Check
- 216 files · ~126,753 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2120 nodes · 5591 edges · 120 communities (100 shown, 20 thin omitted)
- Extraction: 64% EXTRACTED · 36% INFERRED · 0% AMBIGUOUS · INFERRED: 2040 edges (avg confidence: 0.54)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `bacddca4`
...
Cohesion: 0.26
Nodes (29): TourEventType, BaseTourEvent, ChunkPayload, CodeRef, GraphQueryResult, IntentProfile, LLMProvider, QAAnswerResponse (+21 more)

### Community 1 - "Community 1"
Cohesion: 0.15
Nodes (26): Any, CodeRef, _claim(), engine(), _patch_read_chunks(), Verifier-loop tests: actionability rubric + retry budget + flagging.  The ground, Queues responses keyed on which prompt is being graded.      The verifier loop i, Bypass DB-backed ``read_chunks`` — grounding loops over them but     the bodies (+18 more)
...
Cohesion: 0.29
Nodes (10): Chunk, LLMProvider, Settings, embed_chunks(), _embed_items(), EmbeddedChunk, True batched embedding orchestration over repository chunks., Return a deterministic normalized vector when the local embedder rejects a chunk (+2 more)
...
Cohesion: 0.07
Nodes (69): _build_fact_bundle_for_test(), _fact_bundle(), Cartographer — produces ``system_map`` Insights from the call graph.  The Cartog, Run the Cartographer once.      Returns the state diff for the LangGraph reducer, Render the fact bundle as a compact text block.      We deliberately avoid prose, Resolve each symbol's CodeRef from graph_metrics's underlying lookup.      The m, _refs_for_symbols(), _resolve_refs() (+61 more)
...
Cohesion: 0.08
Nodes (31): AsyncEngine, Settings, QAClaim, A single grounded claim in the Q&A answer., Product-layer tables for sessions, entitlements, usage, and tours., answer_deterministically(), build_claims_for_symbols(), build_mermaid() (+23 more)

### Community 13 - "Community 13"
Cohesion: 0.06
Nodes (57): ProviderError, All providers in the fallback chain failed., AsyncEngine, ChunkContent, LLMProvider, Any, MonkeyPatch, Any (+49 more)
...
Cohesion: 0.16
Nodes (27): ArchaeologistState, Any, AsyncEngine, IntentProfile, LLMProvider, build_graph(), _capability_planner_node(), _cartographer_node() (+19 more)
...
Cohesion: 0.20
Nodes (17): AppServices, Any, BaseModel, FastAPI, ArchaeologistError, Every fail-edge in the graph emits one of these., FastAPI app for the Phase 4 API contract., FastAPI app entrypoint. Endpoints are added in Phase 4. (+9 more)
...
Cohesion: 0.14
Nodes (33): ClaimStatus, AsyncEngine, Claim, IntentProfile, LLMProvider, TourSection, CodeRef, MonkeyPatch (+25 more)
...
Cohesion: 0.17
Nodes (14): ChunkContent, CodeRef, ChunkContent, MonkeyPatch, attribute_refs(), Return the refs of the ``k`` chunks most relevant to ``claim_text``.      Best-f, _chunk(), Claim→ref attribution via cross-encoder (stubbed — no model download). (+6 more)

### Community 41 - "Community 41"
Cohesion: 0.10
Nodes (29): EmbeddedChunk, AsyncEngine, Chunk, LLMProvider, Message, Settings, Path, Chunk (+21 more)
...
Cohesion: 0.14
Nodes (18): lane_c_language_violation(), _matches_focus(), Lane C — guarded structural suspicions., Return the banned phrase when Lane C language is too certain., Build guarded suspicion opportunities from deterministic candidates.      Phase, run_lane_c_suspicion(), CodeRef, IntentProfile (+10 more)

### Community 50 - "Community 50"
Cohesion: 0.14
Nodes (14): Connection, _cache_key(), _embedding_cache_key(), EmbeddingResponse, The single LLMProvider every agent goes through.  Responsibilities (Phase 0 deli, Provider-agnostic embedding shape., Thread-safe SQLite cache keyed on the canonical request hash., Generate a completion. Hits cache first; otherwise walks the fallback chain. (+6 more)

### Community 51 - "Community 51"
Cohesion: 0.08
Nodes (41): Path, Path, Path, EmbeddingResponse, Path, ParsedFile, ParsedSymbol, _build_enriched_text() (+33 more)

### Community 52 - "Community 52"
Cohesion: 0.19
Nodes (14): CrossEncoderReranker, ChunkContent, ChunkHit, MonkeyPatch, _hit_and_content(), RAG Phase 4: cross-encoder wrapper + rerank pipeline (stubbed encoder).  The rea, Scores by keyword overlap with the query; counts calls., reranker() (+6 more)
...
Cohesion: 0.15
Nodes (17): AsyncEngine, Settings, Settings, Settings, take_rows(), build_eval_context(), EvalContext, Shared runtime helpers for eval runners. (+9 more)
...
Cohesion: 0.18
Nodes (10): ChunkContent, ChunkContent, ChunkHit, The text the cross-encoder sees for one chunk (symbol-prefixed)., rerank_text(), ``rerank_and_diversify`` — compose cross-encoder + MMR over a hit pool.  Input:, Cross-encoder rerank + MMR diversify; returns top-``k`` best-first.      ``hits`, rerank_and_diversify() (+2 more)
...
Cohesion: 0.21
Nodes (7): Any, CrossEncoderReranker, Cross-encoder reranker over (query, chunk) pairs via ``fastembed``.  Unlike the, Lazy-loading, score-caching wrapper around ``fastembed.TextCrossEncoder``., Relevance score per text (higher = more relevant). Cached per pair., Process-wide reranker so the ONNX model loads once., shared_reranker()
...
Cohesion: 0.20
Nodes (6): Path, Settings, Shared fixtures for the core package's tests., FakeEmbedder, Test double for the sentence-transformers in-process embedder.      Returns dete, tmp_settings()

### Community 62 - "Community 62"
Cohesion: 0.06
Nodes (35): AsyncEngine, ChunkContent, CodeRef, ChunkHit, CodeRef, QuerySpec, Settings, CodeRef (+27 more)
...
Cohesion: 0.27
Nodes (36): Any, AppServices, BaseTourEvent, ChunkPayload, IntentProfile, MonkeyPatch, QAAnswerResponse, RepoStatus (+28 more)

### Community 100 - "Community 100"
Cohesion: 0.67
Nodes (3): _parse_retry_after(), Parse a ``Retry-After`` header (delta-seconds or HTTP-date) into seconds.      P, test_parse_retry_after_variants()

### Community 113 - "Community 113"
Cohesion: 0.08
Nodes (15): Alembic environment — uses Settings.postgres_dsn so dev + CI agree., Path, Shared core: settings, logging, and the LLMProvider abstraction., _find_repo_env(), Application settings, loaded from environment / `.env` via pydantic-settings., Walk up from this file to the repo root and return the ``.env`` path.      Lets, Parse a comma-separated env var into a cleaned list., _split_csv() (+7 more)
...
Cohesion: 0.24
Nodes (8): _BaseClient, _OpenAICompatibleClient, Common interface for provider HTTP shims., Speaks the OpenAI chat-completions shape. Used for Groq and Cerebras., Default wiring used by the app. Tests pass `clients` for full control., AsyncClient, Settings, Self
...
- **Why does `LLMProvider` connect `Community 9` to `Community 0`, `Community 1`, `Community 4`, `Community 5`, `Community 6`, `Community 11`, `Community 12`, `Community 13`, `Community 15`, `Community 16`, `Community 17`, `Community 23`, `Community 24`, `Community 28`, `Community 29`, `Community 35`, `Community 36`, `Community 41`, `Community 45`, `Community 47`, `Community 49`, `Community 50`, `Community 55`, `Community 61`, `Community 72`, `Community 85`, `Community 121`?**
  _High betweenness centrality (0.124) - this node is a cross-community bridge._
- **Why does `Settings` connect `Community 85` to `Community 0`, `Community 4`, `Community 5`, `Community 9`, `Community 10`, `Community 12`, `Community 13`, `Community 15`, `Community 16`, `Community 24`, `Community 25`, `Community 27`, `Community 33`, `Community 35`, `Community 41`, `Community 50`, `Community 51`, `Community 55`, `Community 61`, `Community 62`, `Community 65`, `Community 113`, `Community 121`?**
  _High betweenness centrality (0.113) - this node is a cross-community bridge._
- **Why does `IntentProfile` connect `Community 49` to `Community 0`, `Community 1`, `Community 36`, `Community 37`, `Community 7`, `Community 72`, `Community 9`, `Community 10`, `Community 43`, `Community 12`, `Community 14`, `Community 17`, `Community 19`, `Community 85`, `Community 23`, `Community 29`, `Community 30`?**
