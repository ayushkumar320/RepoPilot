# Graph Report - RepoPilot  (2026-07-06)

## Corpus Check
- 157 files · ~94,274 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1783 nodes · 4535 edges · 126 communities (95 shown, 31 thin omitted)
- Extraction: 62% EXTRACTED · 38% INFERRED · 0% AMBIGUOUS · INFERRED: 1702 edges (avg confidence: 0.53)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a9e9ee3f`
...
Cohesion: 0.15
Nodes (19): EmbeddedChunk, AsyncEngine, Settings, SQLAlchemy schema for Phase 1 ingestion.  Tables:     repos              one row, EmbeddedChunk, Batched async embedder over chunks via the central ``LLMProvider``.  The provide, Return a deterministic normalized vector when the local embedder rejects a chunk, _stable_fallback_vector() (+11 more)

### Community 15 - "Community 15"
Cohesion: 0.15
Nodes (11): ChunkHit, Settings, mrr(), ndcg_at_k(), Pure-retrieval metrics: recall@k, NDCG@k, MRR over a labeled QA dataset.  Runs t, recall_at_k(), RetrievalCaseResult, RetrievalEvalMetrics (+3 more)
...
Cohesion: 0.13
Nodes (18): Any, index_repo(), arq worker function for the Phase 1 ingestion pipeline.  The actual pipeline log, Build arq RedisSettings from Settings.redis_url.      Without this, arq falls ba, arq job: index a GitHub repo end-to-end. Returns a JSON-able status dict., arq discovery target. Run with: ``arq repopilot_api.jobs.index_repo.WorkerSettin, _redis_settings_from_url(), shutdown() (+10 more)
...
Cohesion: 0.29
Nodes (7): Current Build Phase, How to advance the phase, Phase 0 facts that feed later phases, Phase 1 — entry state (all Phase 0 exit criteria hold), The improvement chain (what each phase fixes, and what it hands the next), What the RAG plan operates on (live on `main`), Why Phase 1 is the next build
...
Cohesion: 0.12
Nodes (14): parse_github_url(), GitHub clone + HEAD-SHA helpers for Phase 1 ingestion.  Two entry points:  * :fu, Return ``(owner, name)`` for a public GitHub URL.      Raises ``ValueError`` for, Return the current default-branch HEAD SHA via ``git ls-remote HEAD``.      Used, remote_head_sha(), Phase 1 — clone -> parse -> chunk -> graph -> embed -> persist., Pure-logic tests around URL parsing and the revisit/staleness contract.  The net, test_parse_github_url_accepts_canonical_forms() (+6 more)
...
Cohesion: 0.17
Nodes (16): CloneResult, ModuleSource, Path, Chunk, LLMProvider, Path, Settings, clone_to_tempdir() (+8 more)
...
Cohesion: 0.19
Nodes (12): Path, AsyncEngine, ChunkHit, LLMProvider, _is_noise(), _load_questions(), main(), propose() (+4 more)
...
Cohesion: 0.16
Nodes (27): Any, IntentProfile, Free-text intent in, structured tilt out. No fixed purpose enum., _claim(), engine(), _patch_read_chunks(), Verifier-loop tests: actionability rubric + retry budget + flagging.  The ground, Queues responses keyed on which prompt is being graded.      The verifier loop i (+19 more)
...
Cohesion: 0.26
Nodes (11): MonkeyPatch, PipelineResult, End-to-end Phase 1 pipeline orchestrator.  Wires: clone → parse → chunk → graph, Decide whether ``repo_url`` is already-current, stale, or unknown.      Cheap —, revisit_status(), Idempotency + staleness — exercised against a stubbed DB and stubbed clone.  The, When the remote HEAD has moved past the indexed SHA → status=stale., _StubEngine (+3 more)
...
Cohesion: 0.13
Nodes (24): EmbeddingResponse, ProviderError, All providers in the fallback chain failed., Provider-agnostic embedding shape., Chunk, LLMProvider, Settings, Chunk (+16 more)
...
Cohesion: 0.22
Nodes (13): _coerce_section(), _collect_refs(), _format_source_bundle(), Teacher — weaves Insights into goal-anchored ``TourSection``s.  The Teacher is t, Run the Teacher once.      Returns ``{"draft_tour": [TourSection, …]}``. Empty l, Index every CodeRef from upstream insights by symbol so the Teacher     can only, run_teacher(), CapabilityPlan (+5 more)
...
Cohesion: 0.23
Nodes (8): Connection, Thread-safe SQLite cache keyed on the canonical request hash., Generate a completion. Hits cache first; otherwise walks the fallback chain., Embed ``text`` via the in-process sentence-transformers embedder.          No HT, _SQLiteCache, ModelId, Path, ProviderName

### Community 51 - "Community 51"
Cohesion: 0.18
Nodes (15): AsyncEngine, Settings, Settings, take_rows(), resolve_repo_id(), Eval runners for the phase gates., run_latency_eval(), _patched_read_chunks() (+7 more)
...
Cohesion: 0.08
Nodes (40): detect_quality_opportunities(), _difficulty(), QualityCandidate, Lane B — deterministic code-health opportunities., Transform deterministic detector hits into unified opportunities., run_lane_b_quality(), IntentProfile, Opportunity (+32 more)
...
Cohesion: 0.12
Nodes (43): ClaimStatus, LLMProvider, Message, Single entrypoint to every LLM call in the system., LLMProvider, OutputShape, CodeRef, AsyncEngine (+35 more)
...
Cohesion: 0.40
Nodes (4): Settings, build_eval_context(), EvalContext, Shared runtime helpers for eval runners.
...
Cohesion: 0.33
Nodes (5): EventDict, Any, _drop_chunk_content(), Structlog setup: JSON renderer in prod/CI, human-friendly renderer in dev/tests., Strip any field carrying raw repo content. Logs must never persist source code.
...
Cohesion: 0.20
Nodes (8): LLMResponse, Provider-agnostic response shape., LLMResponse, Any, EmbeddingResponse, LLMProvider, FakeEmbedder, Test double for the sentence-transformers in-process embedder.      Returns dete
...
- **Why does `LLMProvider` connect `Community 67` to `Community 9`, `Community 12`, `Community 14`, `Community 19`, `Community 26`, `Community 27`, `Community 28`, `Community 30`, `Community 31`, `Community 34`, `Community 36`, `Community 39`, `Community 45`, `Community 48`, `Community 50`, `Community 51`, `Community 63`, `Community 64`, `Community 69`, `Community 74`, `Community 75`, `Community 76`, `Community 84`, `Community 96`?**
  _High betweenness centrality (0.090) - this node is a cross-community bridge._
- **Why does `Settings` connect `Community 30` to `Community 96`, `Community 67`, `Community 36`, `Community 9`, `Community 75`, `Community 76`, `Community 45`, `Community 14`, `Community 15`, `Community 50`, `Community 51`, `Community 84`, `Community 53`, `Community 20`, `Community 19`, `Community 52`, `Community 25`, `Community 26`?**
  _High betweenness centrality (0.073) - this node is a cross-community bridge._
- **Why does `IntentProfile` connect `Community 34` to `Community 64`, `Community 66`, `Community 67`, `Community 39`, `Community 8`, `Community 72`, `Community 74`, `Community 12`, `Community 48`, `Community 80`, `Community 82`, `Community 57`, `Community 27`, `Community 30`?**
