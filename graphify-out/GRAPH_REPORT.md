# Graph Report - RepoPilot  (2026-07-13)

## Corpus Check
- 205 files · ~122,880 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2059 nodes · 5051 edges · 125 communities (106 shown, 19 thin omitted)
- Extraction: 64% EXTRACTED · 36% INFERRED · 0% AMBIGUOUS · INFERRED: 1801 edges (avg confidence: 0.54)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e4c34572`
...
Cohesion: 0.12
Nodes (39): _extract_openai_compatible_text(), RateLimitError, Extract assistant text from OpenAI-compatible payload variants.      Some provid, HTTP 429 from a provider — triggers retry/fallback inside the provider.      ``r, LLMResponse, LLMProvider, ProviderName, Message (+31 more)

### Community 4 - "Community 4"
Cohesion: 0.15
Nodes (12): Connection, Thread-safe SQLite cache keyed on the canonical request hash., Default wiring used by the app. Tests pass `clients` for full control., Generate a completion. Hits cache first; otherwise walks the fallback chain., Embed ``text`` via the in-process sentence-transformers embedder.          No HT, _SQLiteCache, ModelId, AsyncClient (+4 more)
...
Cohesion: 0.11
Nodes (18): AsyncEngine, ChunkHit, Any, RAG Phase 3: ``bm25_search`` SQL composition + param plumbing.  The Postgres FTS, _RecordingConn, _RecordingEngine, test_blank_query_and_zero_k_short_circuit(), test_clean_query_strips_stopwords_keeps_identifiers() (+10 more)

### Community 7 - "Community 7"
Cohesion: 0.11
Nodes (35): approachability_score(), Lane A — issue triage backed by graph approachability., Score issue approachability from graph facts, not GitHub labels., Rank issues and keep the next three rejected reasons., _ref_for_issue(), run_lane_a_triage(), triage_issues(), detect_quality_opportunities() (+27 more)

### Community 8 - "Community 8"
Cohesion: 0.11
Nodes (30): Path, Path, Path, ParsedFile, ParsedSymbol, chunk_file(), _class_header_content(), _class_header_end_line() (+22 more)

### Community 9 - "Community 9"
Cohesion: 0.12
Nodes (31): _coerce_section(), _collect_refs(), _format_source_bundle(), Teacher — weaves Insights into goal-anchored ``TourSection``s.  The Teacher is t, Run the Teacher once.      Returns ``{"draft_tour": [TourSection, …]}``. Empty l, Index every CodeRef from upstream insights by symbol so the Teacher     can only, run_teacher(), CapabilityPlan (+23 more)
...
Cohesion: 0.16
Nodes (24): AsyncEngine, ChunkContent, Claim, LLMProvider, answer_question(), _Context, _estimate_tokens(), _extend_context() (+16 more)
...
Cohesion: 0.12
Nodes (20): BaseSettings, EmbeddingResponse, Provider-agnostic embedding shape., Any, EmbeddingResponse, Path, Settings, Shared fixtures for the core package's tests. (+12 more)
...
Cohesion: 0.13
Nodes (15): Agent Graph, API Surface, Architecture At A Glance, Current Build State, Design Principles, Development Workflow, Documentation Map, Graph Connections That Matter (+7 more)
...
Cohesion: 0.10
Nodes (20): Architecture, Design invariants, One-paragraph summary, Part 1 — Layman Explanation, Part 2 — Technical Explanation, Read next, The `_before`/`_after`/`delta` protocol, The "before and after" trick (+12 more)
...
Cohesion: 0.23
Nodes (18): CloneResult, ModuleSource, Chunk, LLMProvider, Path, Settings, Chunk, One indexable unit of source. Line numbers are 1-based, inclusive. (+10 more)
...
Cohesion: 0.21
Nodes (18): ChunkHit, ChunkHit, Reciprocal Rank Fusion (RRF) — fuse multiple ranked ``ChunkHit`` lists.  Used by, Fuse ranked hit lists into one ranking, best-first.      Each input pool must al, reciprocal_rank_fusion(), _ref_key(), _hit(), RAG Phase 3: RRF fusion ordering (``qa/union.py``).  The DB-touching lanes (``bm (+10 more)

### Community 44 - "Community 44"
Cohesion: 0.12
Nodes (17): Current Build Phase, Design decisions & deviations from spec, How it landed — the λ × pool sweep, How to advance the phase, LLM guardrails through the reranked path (closed 2026-07-10), Pending (does not block the land), Phase 0 facts that feed later phases, Phase 1 — how it landed (httpx) (+9 more)
...
Cohesion: 0.22
Nodes (12): _apply(), _Cache, _objection_if_rejected(), Per-claim grounding check against ``read_chunks``.  The Verifier is the single l, Test helper — clear the verifier verdict cache., Verify one claim. Updates ``claim.status`` and ``claim.verifier_note`` in place., The structured response we require from the verifier model., In-process cache keyed by sha256(claim_text + chunk_hashes). (+4 more)
...
Cohesion: 0.22
Nodes (11): Settings, build_eval_context(), EvalContext, Shared runtime helpers for eval runners., resolve_repo_id(), _patched_read_chunks(), Verifier-quality eval runner for the Phase 2 gate., run_verifier_eval() (+3 more)

### Community 54 - "Community 54"
Cohesion: 0.08
Nodes (37): ClaimStatus, _cache_key(), LLMProvider, Message, ProviderError, Single entrypoint to every LLM call in the system., All providers in the fallback chain failed., Modality (+29 more)
...
Cohesion: 0.13
Nodes (14): ChunkHit, Settings, _apply_rerank(), mrr(), ndcg_at_k(), Pure-retrieval metrics: recall@k, NDCG@k, MRR over a labeled QA dataset.  Runs t, Rerank the pool head; keep the tail in original order so recall@k holds., recall_at_k() (+6 more)
...
Cohesion: 0.08
Nodes (53): AsyncEngine, Claim, IntentProfile, LLMProvider, TourSection, Any, CodeRef, TourSection (+45 more)
...
Cohesion: 0.25
Nodes (8): 1. Clone and install, 2. Create `.env`, 3. Start data services, 4. Run the API, 5. Run the web app, 6. Run checks, Prerequisites, Setup Your Own Local Instance
...
Cohesion: 0.40
Nodes (5): Build arq RedisSettings from Settings.redis_url.      Without this, arq falls ba, _redis_settings_from_url(), RedisSettings, get_settings(), Cached settings accessor. Tests should call `get_settings.cache_clear()`.
...
Cohesion: 0.16
Nodes (21): lane_c_language_violation(), _matches_focus(), Lane C — guarded structural suspicions., Return the banned phrase when Lane C language is too certain., Build guarded suspicion opportunities from deterministic candidates.      Phase, run_lane_c_suspicion(), CodeRef, IntentProfile (+13 more)
...
## Knowledge Gaps
- **345 isolated node(s):** `nextConfig`, `name`, `private`, `version`, `type` (+340 more)
...
- **Why does `LLMProvider` connect `Community 54` to `Community 0`, `Community 3`, `Community 4`, `Community 5`, `Community 9`, `Community 11`, `Community 12`, `Community 13`, `Community 16`, `Community 18`, `Community 19`, `Community 21`, `Community 24`, `Community 29`, `Community 41`, `Community 48`, `Community 53`, `Community 70`, `Community 71`, `Community 78`, `Community 88`?**
  _High betweenness centrality (0.104) - this node is a cross-community bridge._
- **Why does `Settings` connect `Community 24` to `Community 0`, `Community 3`, `Community 4`, `Community 70`, `Community 71`, `Community 41`, `Community 15`, `Community 51`, `Community 53`, `Community 54`, `Community 85`, `Community 88`, `Community 25`, `Community 27`, `Community 61`?**
  _High betweenness centrality (0.075) - this node is a cross-community bridge._
- **Why does `IntentProfile` connect `Community 9` to `Community 0`, `Community 5`, `Community 37`, `Community 7`, `Community 135`, `Community 137`, `Community 13`, `Community 14`, `Community 78`, `Community 18`, `Community 19`, `Community 54`, `Community 29`, `Community 30`?**
