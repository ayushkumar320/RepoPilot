...
ANSWER_SYSTEM = (
    ...
    "SHAPE — answer the question first, then support it:\n"
    "- Open with a '## Answer' header, then 1-3 lines that answer the question "
    "as asked, in plain words. A yes/no question gets yes or no on the first "
    "line. A 'why' question gets the reason on the first line.\n"
    "- Then 1-3 sections whose headers name what the question was about, not "
    "the repository: '## How a request reaches the view', never "
    "'## Introduction to the codebase'.\n"
    "- Every line must move that answer forward. Never describe a chunk just "
    "because it was retrieved — a chunk irrelevant to the question is left "
    "out, not summarized.\n"
    "- One idea per line, plain sentences, no stacked clauses. The reader is "
    "reading this once.\n"
    "- Write 6-14 claim lines: simple lookups near 6, how/why questions near 14.\n"
    "- A header line makes no claim and carries no citation.\n"
...
def answer_user_prompt(question: str, chunks: Sequence[ChunkContent]) -> str:
    return (
        f"QUESTION:\n{question}\n\n"
        f"CHUNKS:\n{_render_chunks(chunks)}\n\n"
        "Write the answer below. Start with '## Answer' and answer the question "
        "as asked before any detail, then 1-3 question-specific sections, then "
        "'## Where to look next'. One claim per line, each directly supported "
        "by a chunk, and you MUST cite the chunk index (e.g. [0]) at the end of "
        "the line."
    )
