...
_SYSTEM_PROMPT = (
    ...
    "chunks support.\n\n"
    "A claim may assert an ABSENCE. Judge it by its scope. A claim scoped to "
    "the chunks themselves ('these chunks contain no retry logic') is "
    "SUPPORTED when the chunks indeed do not contain it — that is a fact "
    "about the chunks, and you can see all of them. A claim scoped to the "
    "whole repository ('the repo has no test for this') is REJECTED: the "
    "chunks are a slice and cannot establish what the rest of the repository "
    "does not hold.\n\n"
