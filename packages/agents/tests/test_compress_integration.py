...
async def test_verifier_sees_full_content_after_compression() -> None:
    ...
    (compressed,) = await compress_chunks("what matters?", [original], provider=cast(Any, provider))
...
async def test_compress_chunks_runs_in_parallel_and_handles_errors() -> None:
    ...
    out = await compress_chunks("q", original, provider=cast(Any, _RaisingProvider()))
